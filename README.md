...
If you wish to use other classification methods, (pre-trained/custom) classifiers and/or databases, have a look at the QIIME2 documentation. We currently only support the use of a different classifier, which can be selected/achieved by changing the path to the classifier in the metaTAXONx `nextflow.config` configuration file.
